...
export const PlaceholderClip = observer(({ track }: PlaceholderClipProps) => {
  ...
  const clipStartInPixels = track.placeholderClipStart!.toSamples() / audioEngine.samplesPerPixel

  useEffect(() => {
    if (audioEngine.state === 'recording') {
      Tone.getTransport().scheduleRepeat(() => {
        const transportPositionInPixels = Tone.Time(Tone.getTransport().seconds).toSamples() / audioEngine.samplesPerPixel
